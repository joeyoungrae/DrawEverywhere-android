...
class AuctionListFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (type) {
            Type.AUCTION -> {
                viewModel.setUserAuctionList()
                viewModel.customAuctionList.getNextData()
            }
            else -> {
                viewModel.setUserAuctionList()
                viewModel.customAuctionList.getNextData()
    ...
    override fun onCreateView(
        ...
        adapter = when (type) {
            Type.AUCTION -> {
                AuctionAdapter(viewModel.customAuctionList, AuctionAdapter.ShowType.WALLET_AUCTION, requestManager, action)
            }
            else -> {
                AuctionAdapter(viewModel.customAuctionList, AuctionAdapter.ShowType.WALLET_AUCTION, requestManager, action)
            }
        ...
        viewModel.customAuctionList.mLiveData.observe(viewLifecycleOwner) {
            if (it.isEmpty()) {
                binding.txtZeroAuction.visibility = View.VISIBLE
                binding.swipeLayout.isEnabled = false
            } else {
                binding.txtZeroAuction.visibility = View.INVISIBLE
    ...
    override fun onStart() {
        ...
        if (prefs != null && prefs.getBoolean("auctionListFragment_update", false)) {
            prefs.edit().remove("auctionListFragment_update").apply()
            adapter.data.refreshData()

            Timber.e("nft Auction 변경 사항 있어서 새로고침")
        }
    }
